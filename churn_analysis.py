...
def prepare_model_data(churn_analysis_df):
    ...
    X = churn_analysis_df.drop(['customer_unique_id', 'churn'], axis=1)
...
def evaluate_model(model, X_test, y_test):
    ...
    # Calcular Average Precision Score (área sob a curva Precision-Recall)
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        feature_importance = pd.DataFrame({
            'Feature': X_test.columns if hasattr(X_test, 'columns') else [f"Feature {i}" for i in range(X_test.shape[1])],
...
def save_model_and_results(model, scaler, metrics, cutoff_date, rebalance_method, model_type, class_weight, feature_columns):
    ...
    with open('churn_analysis_results.txt', 'w') as f:
        ...
        if feature_importance is not None:
            f.write(f"Importância das features:\n")
